...
{
    /// <summary>
    /// 各AIツールへのMCPサーバー登録・解除を管理するクラス（基本版）。
    /// プロジェクト固有の設定（トークン、ポート等）を含まないシンプルな登録を行います。
    ///
    /// プロジェクト固有の設定を含む登録が必要な場合は、McpProjectRegistryを使用してください。
    ///
    ...
    public static class McpToolRegistry
    {
        ...
        private static void UnregisterViaConfig(AITool tool)
        {
            ...
            {
                var projectPath = McpProjectRegistry.GetProjectPath();
                McpConfigManager.RemoveServerFromClaudeCode(config, projectPath, McpServerManager.ServerName);
            }
            else
            {
                if (config.ContainsKey("mcpServers"))
        ...
        public static bool IsRegistered(AITool tool)
        {
            try
            {
                ...
                {
                    var projectPath = McpProjectRegistry.GetProjectPath();
                    return McpConfigManager.IsServerRegisteredInClaudeCode(config, projectPath, McpServerManager.ServerName);
                }
